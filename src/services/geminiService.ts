...
async function fetchFromGemini(retryCount = 0): Promise<CommodityReportData> {
  ...
  const prompt = `
    ...
    [링크 무결성 검증 단계]
    각 기사에 대해 반드시 다음 검증을 수행한다.
    1. URL이 실제 기사 페이지인지 확인
    2. 기사 제목과 기사 본문 내용이 일치하는지 확인
    3. 광고 페이지 / 리다이렉트 페이지 제외
    4. 로그인 필요한 페이지 제외
    5. 접근 불가 링크 (404 / 차단 페이지) 제외
    반드시 "검증된 원본 URL"만 사용한다.
    ...
    4. 기사 제목과 본문 내용 일치
    5. 중복 기사 없음
    6. 오늘 또는 지난 24시간 내 기사
    조건이 충족되지 않을 경우 추가 검색 후 데이터를 보완한다.

    [최종 목표]
    신뢰도 높은 글로벌 원자재 뉴스 요약 데이터를 생성하고 각 기사에 대해 검증된 원문 URL을 제공한다.
